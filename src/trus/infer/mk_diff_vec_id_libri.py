...
# ========== modify here ==========
FORGET_ROOT     = Path("<Path for forget activation dir>")
REMAIN_MEAN_DIR = Path("<Path for remain mean activation dir")
OUT_ROOT        = Path("<Path for saving diff activation dir")


NUM_LAYERS   = 22
START_LAYER  = 1
MEAN_PREFIX  = "remain_50_mean_layer_"   # Average file prefix
MEAN_SUFFIX  = ".npy"                    # average file extension
OVERWRITE    = True                      
SAVE_FLAT_UNIT = False                   # If True, 1D (flat) unit vectors are also stored additionally.
EPS = 1e-8
# ========================================
...
def load_forget_layer(sample_dir: Path, sample_id: str, layer_idx: int) -> np.ndarray:
    # basic rules: {ID}_layer_{i}.npy (i는 1..NUM_LAYERS)
