...
# ===== (Libri) cross sentence list / audio root =====
LIBRI_LST   = "<PATH for librispeech_pc_test_clean_cross_sentence.lst>"
LIBRI_ROOT  = "<Audio path LibriSpeech/test-clean_unseen>"

# ===== number of remain speakers =====
REMAIN_NUM       = 10  # {10, 30, 50}
FORGET_ROOT      = Path("<Activation path for forget_unseen set>")
REMAIN_MEAN_DIR  = Path("<Activation path for remain mean>")
DIFF_ROOT        = Path("<Activation path for diff_libri")
...
# ===== output path =====
RUN_TAG_BASE     = "Name you want"
RUN_TAG          = f"{RUN_TAG_BASE}_r{REMAIN_NUM}"
STEERED_OUT_DIR  = Path(f"<PATH for result>/{RUN_TAG}_{LAYER_BAND_MODE}")

# save selection results (option)
...
def load_diffs_for_sample(sample_diff_dir: Path, layers: List[int], nfe_steps: int) -> Dict[int, np.ndarray]:
    ...
    for li in layers:
        cands = [
            ...
            sample_diff_dir / "diff_stack.npy",  # 3D stack support
        ]
        arr = None
        for p in cands:
            if p.is_file():
                if p.name == "diff_stack.npy":
                    st = np.load(str(p))
                    if st.ndim == 3 and 1 <= li <= st.shape[0]:
                        arr = st[li - 1]
                        break
                else:
                    arr = np.load(str(p))
...
from f5_tts.eval.utils_eval import get_librispeech_test_clean_metainfo
...
def main():
    ...
    # meta data for Libri 
    meta = get_librispeech_test_clean_metainfo(LIBRI_LST, LIBRI_ROOT)
    print(f"[INFO] Libri meta loaded: {len(meta)} samples")

    ok_cnt, skip_cnt = 0, 0

    for (utt, prompt_text, prompt_wav, gt_text, gt_wav) in tqdm(meta, desc="[STEER-LIBRI-ABL]"):
        sid = utt

        # (A) forget / remain_mean activaion file mapping
        ...
        # (B) calculate step-wise cos per layer → utilize average of layer 
        ...
        # (C) layer selection considering variation (4 modes of band)
        ...
        # (E) steering diff load (only selected layer)
        ...
        # (F) jook setting(inject into FFN-out through subtracting erase_proj vector)
        ...
        # (G) synthesis
        ref_audio = str(Path(LIBRI_ROOT) / prompt_wav)
        ...
        # (I) save selection results as the form of JSON (optional)
        if SAVE_PICKED_JSON:
