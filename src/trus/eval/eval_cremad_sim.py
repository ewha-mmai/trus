...
def main():
    ...
    if use_funasr:
        if FunASRModel is None:
            raise RuntimeError("[WARN] funasr is not installed")
        ...
        target_sr = int(default_sr) if (args.target_sr is None) else args.target_sr
    ...
    for ex in items:
        ...
        gen_path = find_gen_audio(args.gen_wav_dir, uid, args.gen_name_tpl)
        if gen_path is None:
